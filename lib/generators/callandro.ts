...
export async function generateCallAndro(nama: string, durasi: string, avatarSrc: string) {
    try {
        const ASSETS_DIR = join(process.cwd(), 'assets', 'call_andro');
        const FONTS_DIR = join(ASSETS_DIR, 'fonts');
        const BG_LOCAL = join(ASSETS_DIR, 'template_call_andro.png');
        ...
        await drawTextWithEmojisCenter(ctx, durasi, 130, 30, `400 30px RobotoWA, sans-serif`);

        return canvas.toBuffer('image/png');

    } catch (error) {
        console.error("Error generating call andro image:", error);
        throw error;
    }
}