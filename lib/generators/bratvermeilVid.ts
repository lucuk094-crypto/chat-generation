...
const ASSETS_DIR = join(process.cwd(), 'public', 'assets', 'bratvermeil');
...
export async function generateBratVermeilVideo(text: string): Promise<Buffer> {
  ...
  const tmpDir = await mkdtemp(join(tmpdir(), "vermile-bratvid-"));
  ...
  try {
    ...
    const framePaths = frames.map((_, index) => {
      return join(tmpDir, `frame-${String(index + 1).padStart(4, "0")}.png`);
    });
