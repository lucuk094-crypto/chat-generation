// Nokia Canvas Generator using external API
export interface NokiaOptions {
  text: string;
  from: string;
  date: string;
  time: string;
...
export async function generateNokiaCanvas(options: NokiaOptions): Promise<string> {
  const { text, from, date, time, title } = options;
  ...
  try {
    ...
    const response = await fetch(
      `https://apii.nexadev.my.id/nokia?${params.toString()}`,
      {
        method: 'GET',
        headers: {
    ...
    console.error('Error generating Nokia canvas:', error);
    throw error;
  }
}
